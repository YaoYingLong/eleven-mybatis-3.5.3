...
public class MapperRegistry {
  ...
  public <T> void addMapper(Class<T> type) {
    if (type.isInterface()) {   // 判断传入进来的type类型是不是接口，若不是接口直接跳过
      if (hasMapper(type)) {    // 判断缓存中有没有该类型，若已经缓存则抛出异常
        throw new BindingException("Type " + type + " is already known to the MapperRegistry.");
      }
      boolean loadCompleted = false;
      try {
        // 创建一个MapperProxyFactory把Mapper接口保存到工厂类中
        ...
        if (!loadCompleted) { // 若未加载完成则从knownMappers中移除
