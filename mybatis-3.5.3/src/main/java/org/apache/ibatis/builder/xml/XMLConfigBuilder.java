...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private XMLConfigBuilder(XPathParser parser, String environment, Properties props) {
    /**
     * 调用父类的BaseBuilder的构造方法给configuration赋值 typeAliasRegistry别名注册器赋值 TypeHandlerRegistry赋值
     */
  ...
  public Configuration parse() {
    if (parsed) { // 若已经解析过了 就抛出异常
      throw new BuilderException("Each XMLConfigBuilder can only be used once.");
    }
    parsed = true; // 设置解析标志位
    /**
     * 解析mybatis-config.xml的<configuration>...</configuration>节点
     */
  ...
  private void parseConfiguration(XNode root) {
    try {
      /**
       * 解析properties节点<properties resource="mybatis/db.properties"/>
       ...
       * 具体可配置哪些属性：http://www.mybatis.org/mybatis-3/zh/configuration.html#settings
       * <settings>
      ...
      /**
       * 基本没有用过该属性，若配置了vfsImpl属性，VFS含义是虚拟文件系统；主要是通过程序能够方便读取本地文件系统、FTP文件系统等系统中的文件资源。
       * Mybatis中提供了VFS这个配置，主要是通过该配置可加载自定义虚拟文件系统应用程序，解析到：org.apache.ibatis.session.Configuration#vfsImpl
       */
      loadCustomVfs(settings); // 配置了vfsImpl属性
      /**
       * 指定MyBatis所用日志具体实现，未指定时将自动查找。
       ...
       * 解析别名
       * <typeAliases>
       *  <typeAlias alias="Author" type="cn.eleven.pojo.Author"/>
       * </typeAliases>
       * <typeAliases>
       *  <package name="cn.eleven.pojo"/>
       * </typeAliases>
